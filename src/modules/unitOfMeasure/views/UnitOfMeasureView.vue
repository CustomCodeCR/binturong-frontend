...
function patchUnitInList(payload: UnitOfMeasureSuccessPayload) {
  const existingIndex = units.value.findIndex((unit) => unit.uomId === payload.uomId);

  if (existingIndex >= 0) {
    replaceUnits(
      units.value.map((unit) =>
        unit.uomId === payload.uomId
          ? { ...unit, uomId: payload.uomId, code: payload.code, name: payload.name, isActive: payload.isActive }
          : unit,
      ),
    );
    return;
  ...
  replaceUnits([
    { id: `uom:${payload.uomId}`, uomId: payload.uomId, code: payload.code, name: payload.name, isActive: payload.isActive } as UnitOfMeasure,
    ...units.value,
  ]);
}

function patchUnitStatusInList(uomId: string, isActive: boolean) {
  replaceUnits(units.value.map((unit) => unit.uomId === uomId ? { ...unit, isActive } : unit));
}
...
function hasUnitReachedExpectedState(fetchedUnits: UnitOfMeasure[], expected: UnitOfMeasureSuccessPayload): boolean {
  const fetchedUnit = fetchedUnits.find((unit) => unit.uomId === expected.uomId);
  if (!fetchedUnit) return false;
  return fetchedUnit.code === expected.code && fetchedUnit.name === expected.name && fetchedUnit.isActive === expected.isActive;
}
...
async function reloadUnitsUntil(
  ...
  try {
    ...
    toastStore.addToast({ severity: "error", title: t("toast.error"), message: t("unitsOfMeasure.messages.loadError") });
  } finally {
    loading.value = false;
  }
}
...
function openCreateModal() {
  modalStore.open({
    ...
    onSuccess: async (payload?: UnitOfMeasureSuccessPayload) => {
      if (payload?.uomId) patchUnitInList(payload);
      toastStore.addToast({ severity: "success", title: t("toast.success"), message: t("unitsOfMeasure.messages.createSuccess") });
      if (payload?.uomId) {
        await reloadUnitsUntil((fetchedUnits) => fetchedUnits.some((unit) => unit.uomId === payload.uomId), { attempts: 12, delayMs: 500 });
        return;
      }
      await loadUnits();
    },
    onError: (error) => {
      toastStore.addToast({ severity: "error", title: t("toast.error"), message: error?.message ?? t("unitsOfMeasure.messages.createError") });
    },
  });
}
...
function openEditModal(unit: UnitOfMeasure) {
  modalStore.open({
    ...
    props: { uomId: unit.uomId },
    onSuccess: async (payload?: UnitOfMeasureSuccessPayload) => {
      if (!payload?.uomId) { await loadUnits(); return; }
      patchUnitInList(payload);
      toastStore.addToast({ severity: "success", title: t("toast.success"), message: t("unitsOfMeasure.messages.updateSuccess") });
      await reloadUnitsUntil((fetchedUnits) => hasUnitReachedExpectedState(fetchedUnits, payload), { attempts: 12, delayMs: 500 });
    },
    onError: (error) => {
      toastStore.addToast({ severity: "error", title: t("toast.error"), message: error?.message ?? t("unitsOfMeasure.messages.updateError") });
    },
  });
}
...
async function toggleUnitStatus(unit: UnitOfMeasure) {
  ...
  try {
    await UnitsOfMeasureService.update(unit.uomId, { code: unit.code, name: unit.name, isActive: nextIsActive });
    patchUnitStatusInList(unit.uomId, nextIsActive);
    toastStore.addToast({
      severity: "success",
      title: t("toast.success"),
      message: unit.isActive ? t("unitsOfMeasure.messages.deactivateSuccess") : t("unitsOfMeasure.messages.reactivateSuccess"),
    });
    await reloadUnitsUntil(
      (fetchedUnits) => fetchedUnits.find((item) => item.uomId === unit.uomId)?.isActive === nextIsActive,
    ...
    toastStore.addToast({
      severity: "error",
      title: t("toast.error"),
      message: unit.isActive ? t("unitsOfMeasure.messages.deactivateError") : t("unitsOfMeasure.messages.reactivateError"),
    });
  }
}
...
async function deleteUnit(unit: UnitOfMeasure) {
  try {
    ...
    toastStore.addToast({ severity: "success", title: t("toast.success"), message: t("unitsOfMeasure.messages.deleteSuccess") });
    await reloadUnitsUntil((fetchedUnits) => !fetchedUnits.some((item) => item.uomId === unit.uomId), { attempts: 12, delayMs: 500 });
  } catch {
    toastStore.addToast({ severity: "error", title: t("toast.error"), message: t("unitsOfMeasure.messages.deleteError") });
  }
}
    ...
    >
      ...
      >
        ...
        <div class="flex flex-col sm:flex-row gap-bt-spacing-12 w-full lg:max-w-2xl">
          ...
          >
            <option value="all">{{ $t("unitsOfMeasure.filters.allStatus") }}</option>
            <option value="active">{{ $t("unitsOfMeasure.filters.active") }}</option>
            <option value="inactive">{{ $t("unitsOfMeasure.filters.inactive") }}</option>
          </select>
        ...
        <table v-else class="w-full border-collapse min-w-[600px]">
          <thead class="sticky top-0 z-10">
            <tr class="bg-bt-primary-50 text-left">
              <th class="px-bt-spacing-16 py-bt-spacing-12 text-bt-primary-700">{{ $t("unitsOfMeasure.table.code") }}</th>
              <th class="px-bt-spacing-16 py-bt-spacing-12 text-bt-primary-700">{{ $t("unitsOfMeasure.table.name") }}</th>
              <th class="px-bt-spacing-16 py-bt-spacing-12 text-bt-primary-700">{{ $t("unitsOfMeasure.table.status") }}</th>
              <th class="px-bt-spacing-16 py-bt-spacing-12 text-bt-primary-700 w-20">{{ $t("unitsOfMeasure.table.options") }}</th>
            </tr>
          </thead>
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-bt-spacing-16 py-bt-spacing-12">
                <span
                  ...
                  :class="unit.isActive ? 'bg-bt-success-100 text-bt-success-700' : 'bg-bt-error-100 text-bt-error-700'"
                >
                  {{ unit.isActive ? $t("unitsOfMeasure.status.active") : $t("unitsOfMeasure.status.inactive") }}
                </span>
              </td>
              <td class="px-bt-spacing-16 py-bt-spacing-12">
                <UnitOfMeasureActionMenu
                  :items="[
                    { label: t('unitsOfMeasure.actions.viewDetails'), action: () => openDetailsDrawer(unit) },
                    { label: t('unitsOfMeasure.actions.edit'), action: () => openEditModal(unit) },
                    { label: unit.isActive ? t('unitsOfMeasure.actions.deactivate') : t('unitsOfMeasure.actions.reactivate'), action: () => toggleUnitStatus(unit), danger: unit.isActive },
                    { label: t('unitsOfMeasure.actions.delete'), action: () => deleteUnit(unit), danger: true },
                  ]"
                >
                  <template #trigger>
                    <button
            ...
            <tr v-if="!filteredUnits.length && !loading">
      ...
      >
        <div class="text-sm text-bt-grey-600">
          ...
          <span class="text-bt-grey-500">
            ({{ filteredUnits.length }} {{ $t("unitsOfMeasure.filtered") }})
          </span>
        </div>
