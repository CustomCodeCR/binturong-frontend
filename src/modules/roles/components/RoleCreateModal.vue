...
function normalizeScopeId(scope: Scope): string {
  return String(scope.scopeId ?? "").trim();
}
...
function selectAllFilteredScopes() {
  const ids = filteredScopes.value.map(normalizeScopeId);
...
async function submit() {
  ...
  try {
    ...
    modalStore.onSuccess?.({
      ...
      description: description.value.trim(),
      isActive: isActive.value,
      scopes: scopeOptions.value.filter((scope) =>
  ...
  >
    <div class="mb-bt-spacing-24">
      ...
      <p class="text-bt-grey-600 mt-bt-spacing-8">
        {{ $t("roles.modal.createDescription") }}
      </p>
    </div>
