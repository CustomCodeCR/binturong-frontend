...
async function loadProduct() {
  ...
  try {
    const response = await ProductsService.readById(props.productId);
    ...
    uomId.value = response.uomId;
    taxId.value = response.taxId;
    basePrice.value = response.basePrice;
    averageCost.value = response.averageCost;
    isService.value = response.isService;
...
async function submit() {
  ...
  if (
    ...
    modalStore.onError?.({
      code: 400,
      message: t("products.validation.requiredUpdate"),
    });
    return;
  }
  ...
  try {
    await ProductsService.update(product.value.productId, {
      sku: sku.value.trim(),
...
onMounted(async () => {
  await Promise.all([loadCatalogs(), loadProduct()]);
});
</script>
  ...
  >
    ...
    <div class="mt-bt-spacing-24 flex justify-end gap-bt-spacing-12">
      ...
      <button
        ...
        @click="submit"
      >
        {{ saving ? $t("common.loading") : $t("products.actions.saveChanges") }}
      </button>
    </div>
  </div>
</template>
