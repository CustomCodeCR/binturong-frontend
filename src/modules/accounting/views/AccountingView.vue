...
const reportRangeInvalid = computed(
  () => !isDateRangeValid(reportFrom.value, reportTo.value),
);
...
/**
 * Valida el rango antes de llamar a la API: no basta con exigir ambas fechas,
 * también hay que rechazar un inicio posterior al fin (el backend devolvía un
 * reporte vacío sin avisar).
 */
function hasValidReportRange(): boolean {
  ...
  if (!isDateRangeValid(reportFrom.value, reportTo.value)) {
    toastStore.addToast({
      severity: "error",
      title: t("toast.error"),
...
async function exportIncomeStatementPdf() {
  ...
  try {
    ...
    await downloadBlob(blob, "income-statement.pdf");
  } catch (error) {
    toastStore.addToast({
      severity: "error",
      title: t("toast.error"),
      message: describeDownloadError(
        error,
        t("accounting.messages.exportIncomeStatementError"),
      ),
    });
  }
}
...
async function exportCashFlowExcel() {
  ...
  try {
    ...
    await downloadBlob(blob, "cash-flow.xlsx");
  } catch (error) {
    toastStore.addToast({
      severity: "error",
      title: t("toast.error"),
      message: describeDownloadError(
        error,
        t("accounting.messages.exportCashFlowError"),
      ),
    });
  }
}
  ...
  <section class="h-full min-h-0 bg-bt-grey-50 p-bt-spacing-24 flex flex-col">
    ...
    >
      ...
      >
        ...
        <div v-if="reportRangeInvalid" class="shrink-0">
