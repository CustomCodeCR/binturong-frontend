...
function mapInvoiceToSelectOption(invoice: Invoice): SelectOption {
  ...
  const baseAmount = formatMoney(invoice.pendingAmount ?? invoice.total ?? 0);

  return {
    id: invoice.invoiceId,
    label: `${reference} - ${client} - ${baseAmount}`,
...
async function loadCatalogs() {
  ...
  try {
    ...
    modalStore.onError?.({
      ...
      message: error?.message ?? t("payments.debitNotes.messages.loadError"),
    });
  } finally {
...
async function loadInvoice() {
  ...
  try {
    ...
    if (isPaidInvoice(invoice)) {
      ...
      modalStore.onError?.({
        code: 400,
        message: t("payments.debitNotes.validation.invoiceAlreadyPaid"),
      });
      return;
    }
    ...
    modalStore.onError?.({
      code: error?.status ?? 500,
      message:
        error?.message ?? t("payments.debitNotes.messages.loadInvoiceError"),
    });
  } finally {
...
async function submit() {
  ...
  if (Number.isNaN(normalizedTotalAmount) || normalizedTotalAmount <= 0) {
    modalStore.onError?.({
      code: 400,
      message: t("payments.debitNotes.validation.amountRequired"),
    });
    return;
  }
  ...
  try {
    const created = await DebitNotesService.create({
    ...
    await DebitNotesService.emit(created.debitNoteId);
    ...
    modalStore.onError?.({
      code: error?.status ?? 500,
      message: error?.message ?? t("payments.debitNotes.messages.createError"),
    });
  } finally {
    loading.value = false;
  }
  ...
  >
    <div class="mb-bt-spacing-24">
      <h2 class="text-xl font-bt-bold text-bt-primary-700">
        {{ $t("payments.debitNotes.modal.title") }}
      </h2>
      <p class="text-bt-grey-600 mt-bt-spacing-8">
        {{ $t("payments.debitNotes.modal.description") }}
      </p>
    </div>
    ...
    <div v-else class="space-y-bt-spacing-16">
      <div>
        <label class="block mb-bt-spacing-8 text-sm text-bt-primary-700">
          {{ $t("payments.debitNotes.fields.invoice") }}
        </label>
        <select
          v-model="invoiceId"
        ...
        >
          <option value="">
            {{ $t("payments.debitNotes.placeholders.selectInvoice") }}
          </option>
          <option
      ...
      >
        <div class="text-sm text-bt-grey-500">
          {{ $t("payments.debitNotes.fields.selectedInvoiceBalance") }}
        </div>
      ...
      <div>
        <label class="block mb-bt-spacing-8 text-sm text-bt-primary-700">
          {{ $t("payments.debitNotes.fields.reason") }}
        </label>
        <textarea
          v-model="reason"
          rows="3"
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-bt-spacing-16">
        <div>
          <label class="block mb-bt-spacing-8 text-sm text-bt-primary-700">
            {{ $t("payments.debitNotes.fields.totalAmount") }}
          </label>
          <input
        ...
        <div>
          <label class="block mb-bt-spacing-8 text-sm text-bt-primary-700">
            {{ $t("payments.debitNotes.fields.issueDate") }}
          </label>
          <input
            v-model="issueDate"
            type="date"
