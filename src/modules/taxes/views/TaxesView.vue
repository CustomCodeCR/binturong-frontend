...
function patchTaxInList(payload: TaxSuccessPayload) {
  const existingIndex = taxes.value.findIndex(
    (tax) => tax.taxId === payload.taxId,
  );
...
function hasTaxReachedExpectedState(
  fetchedTaxes: Tax[],
  expected: TaxSuccessPayload,
): boolean {
  ...
  modalStore.open({
    ...
    onSuccess: async (payload?: TaxSuccessPayload) => {
      ...
      toastStore.addToast({
        ...
        message: t("taxes.messages.createSuccess"),
      });

      if (payload?.taxId) {
        await reloadTaxesUntil(
          (fetchedTaxes) =>
            fetchedTaxes.some((tax) => tax.taxId === payload.taxId),
          {
            attempts: 12,
            delayMs: 500,
          },
    ...
    onError: (error) => {
      toastStore.addToast({
        severity: "error",
        title: t("toast.error"),
        message: error?.message ?? t("taxes.messages.createError"),
      });
    },
  });
}
  ...
  modalStore.open({
    ...
    onSuccess: async (payload?: TaxSuccessPayload) => {
      ...
      toastStore.addToast({
        severity: "success",
        title: t("toast.success"),
        message: t("taxes.messages.updateSuccess"),
      });
...
async function toggleTaxStatus(tax: Tax) {
  ...
  try {
    ...
    toastStore.addToast({
      severity: "success",
      title: t("toast.success"),
      message: tax.isActive
        ? t("taxes.messages.deactivateSuccess")
        : t("taxes.messages.reactivateSuccess"),
    });

    await reloadTaxesUntil(
      (fetchedTaxes) => {
        const fetchedTax = fetchedTaxes.find(
          (item) => item.taxId === tax.taxId,
        );
        return fetchedTax?.isActive === nextIsActive;
      },
      {
        attempts: 12,
        delayMs: 500,
    ...
    toastStore.addToast({
      ...
      title: t("toast.error"),
      message: tax.isActive
        ? t("taxes.messages.deactivateError")
        : t("taxes.messages.reactivateError"),
    });
  }
}
...
async function deleteTax(tax: Tax) {
  try {
    ...
    toastStore.addToast({
      severity: "success",
      title: t("toast.success"),
      message: t("taxes.messages.deleteSuccess"),
    });
    ...
    toastStore.addToast({
      severity: "error",
      title: t("toast.error"),
      message: t("taxes.messages.deleteError"),
    });
  }
}
            ...
            >
              ...
              <td class="px-bt-spacing-16 py-bt-spacing-12">
                <TaxActionMenu
                  :items="[
                    {
                      label: t('taxes.actions.viewDetails'),
                      action: () => openDetailsDrawer(tax),
                    },
                    {
                      label: t('taxes.actions.edit'),
                      action: () => openEditModal(tax),
                    },
                    {
                      label: tax.isActive
                        ? t('taxes.actions.deactivate')
                        : t('taxes.actions.reactivate'),
                      action: () => toggleTaxStatus(tax),
                      danger: tax.isActive,
                    },
                    {
                      label: t('taxes.actions.delete'),
