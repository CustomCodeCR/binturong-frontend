...
import type { SupplierContact } from "@/core/interfaces/suppliers";
...
async function submit() {
  if (
    ...
    modalStore.onError?.({
      code: 400,
      message: t("suppliers.contacts.validation.required"),
    });
    return;
  }
  ...
  try {
    if (isEdit && props.contact?.contactId) {
      await SupplierContactsService.update(
        props.supplierId,
      ...
      await SupplierContactsService.create(props.supplierId, {
        name: name.value.trim(),
        jobTitle: jobTitle.value.trim(),
        email: email.value.trim(),
        phone: phone.value.trim(),
    ...
    modalStore.onError?.({
      ...
      message:
        error?.message ??
        (isEdit
          ? t("suppliers.contacts.messages.updateError")
          : t("suppliers.contacts.messages.createError")),
    });
  } finally {
    loading.value = false;
  }
  ...
  >
    <div class="mb-bt-spacing-24">
      <h2 class="text-xl font-bt-bold text-bt-primary-700">
        {{
          isEdit
            ? $t("suppliers.contacts.modal.editTitle")
            : $t("suppliers.contacts.modal.createTitle")
        }}
      </h2>
      <p class="text-bt-grey-600 mt-bt-spacing-8">
        {{
          isEdit
            ? $t("suppliers.contacts.modal.editDescription")
            : $t("suppliers.contacts.modal.createDescription")
        }}
      </p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-bt-spacing-16">
      <div>
        <label class="block mb-bt-spacing-8 text-sm text-bt-primary-700">{{
          $t("suppliers.contacts.fields.name")
        }}</label>
        <input
          v-model="name"
          type="text"
      ...
      <div>
        <label class="block mb-bt-spacing-8 text-sm text-bt-primary-700">{{
          $t("suppliers.contacts.fields.jobTitle")
        }}</label>
        <input
          v-model="jobTitle"
          type="text"
      ...
      <div>
        <label class="block mb-bt-spacing-8 text-sm text-bt-primary-700">{{
          $t("suppliers.contacts.fields.email")
        }}</label>
        <input
          v-model="email"
          type="email"
      ...
      <div>
        <label class="block mb-bt-spacing-8 text-sm text-bt-primary-700">{{
          $t("suppliers.contacts.fields.phone")
        }}</label>
        <input
          v-model="phone"
          type="text"
      ...
      <div class="md:col-span-2 flex items-center gap-bt-spacing-8">
        ...
        <span class="text-bt-primary-700">{{
          $t("suppliers.contacts.fields.isPrimary")
        }}</span>
      </div>
    </div>
